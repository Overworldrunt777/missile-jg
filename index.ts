...
class BootLevel extends Phaser.Scene {
  ...
  preload() {
    ...
    this.load.baseURL =
    "https://overworldrunt777.github.io/strawberry-of-power/"
    this.load.bitmapFont({
      key: 'Oswald',
      textureURL: 'static/assets/font/OswaldLightRed.png',
      fontDataURL: 'static/assets/font/OswaldLightRed.xml',
    });
    this.load.image('logo', 'static/assets/strawberry of power.png');
...
class SplashLevel extends Phaser.Scene {
  ...
  preload() {
    ...
    const logo = this.add.image(225, 115, 'logo');
    logo.setScale(6);
    this.logo = logo;

    const text1 = this.add.bitmapText(-300, 200, 'Oswald', 'Strawberry', 32);
    this.companyLine1 = text1;
    const text2 = this.add.bitmapText(-300, 230, 'Oswald', 'Ultimate Games', 32);
